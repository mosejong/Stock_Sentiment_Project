...
즉, `run_pipeline.py`는  
**데이터 갱신 → AI 분석 → 성과 평가**를 하나의 흐름으로 실행하는 상위 파이프라인 역할을 합니다.
...
├── run_pipeline.py              # 🔄 데이터 업데이트 → AI 분석 → 평가 자동 실행
├── logs/
...
현재 버전에서 뉴스 중요도는  
**명시적인 수치 가중치 방식이 아니라, Gemini가 핵심 뉴스를 요약하는 과정에서 암묵적으로 반영되는 구조**입니다.

즉,
- A 뉴스가 B 뉴스보다 훨씬 중요하더라도
- 현재 구조에서는 LLM이 이를 자연어적으로 우선 반영하는 수준이며
